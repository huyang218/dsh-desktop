...
| **内置工具链** | 打包版自带 Node 运行时,目标机器无需预装任何东西。源码运行时回退为在本机查找 Node ≥ 22(PATH、nvm、Homebrew、`%ProgramFiles%`),这也顺带绕开了 GUI 应用不继承 shell `PATH` 的问题。同一批目录也会追加进每个子进程拿到的 `PATH`,所以 `dsh` 指望在那里找到的工具——装插件用的 pnpm、可委派的 Claude Code 与 Codex CLI——在双击启动的应用里也看得见,而不只是在终端里。 |
