/**
 * `dsh-miniapp` — the simulator on a command line.
 *
 ...
 * lines, the page's elements one per line, no decoration.
 *
...
import { emptyLog, logLine, OPS, parseArgs } from './miniapp-ops.js'
...
const op = OPS[verb]
if (!op) {
  process.stderr.write(`dsh-miniapp: no such command "${verb}"\n\n${usage()}`)
  process.exit(2)
}
...
if (parsed.error) {
  process.stderr.write(`dsh-miniapp ${verb}: ${parsed.error}\n`)
  process.exit(2)
}
...
for (const name of ['project', 'path']) {
...
function usage() {
  const lines = ['usage: dsh-miniapp <command> [arguments] [--options]', '']
...
function render(verb, result) {
  if (!result.ok) return `dsh-miniapp ${verb}: ${result.why ?? 'failed'}${place(result)}\n`
  const lines = []
  ...
  if (result.png) {
    ...
    const file = path.resolve(process.cwd(), 'miniapp.png')
...
/** A failure that knows where it happened says so; most do not. */
function place(result) {
  return result.route ? ` (on ${result.route})` : ''
}
