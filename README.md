...
| **Bundled toolchain** | Packaged builds carry their own Node runtime, so a target machine needs nothing preinstalled. Running from source falls back to finding Node ≥ 22 on the machine (PATH, nvm, Homebrew, `%ProgramFiles%`), which also sidesteps GUI apps not inheriting a shell `PATH`. The same directories are appended to the `PATH` every child gets, so tools `dsh` expects to find there — pnpm for plugins, the Claude Code and Codex CLIs it can delegate to — are visible to a double-clicked app and not only in a terminal. |
